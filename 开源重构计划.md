...
**完成状态**: ✅ 已完成
- `hsb_color_wheel.py` → `color_wheel.py`（类名保持 `HSBColorWheel`）
- `histogram_widget.py` + `luminance_histogram.py` → `luminance_histogram_widget.py`
- 类名从 `HistogramWidget` 改为 `LuminanceHistogramWidget`，更明确表达用途
...
| `color_wheel.py` | `HSBColorWheel` | ✓ |
| `luminance_histogram_widget.py` | `LuminanceHistogramWidget` | ✓ |
| `rgb_histogram_widget.py` | `RGBHistogramWidget` | ✓ |

**完成状态**: ✅ 已完成
- 所有类名符合 PascalCase 规范
- `HistogramWidget` 已重命名为 `LuminanceHistogramWidget`，与 `RGBHistogramWidget` 保持一致风格
...
**检查清单**:
- [x] `image_canvas.py` 中的私有属性
- [x] `luminance_canvas.py` 中的私有属性
- [x] `color_card.py` 中的私有属性
- [x] `luminance_card.py` 中的私有属性

**完成状态**: ✅ 已完成
- 所有私有属性和方法已使用单下划线前缀 `_`
- 已合并的两个直方图组件中的私有成员也已统一
...
| 任务 | 状态 | 主要工作 |
|:---|:---:|:---|
| 3.1 统一文件名 | ✅ | 重命名 `hsb_color_wheel.py` → `color_wheel.py`，合并并重命名直方图组件 |
| 3.2 统一类名 | ✅ | `HistogramWidget` → `LuminanceHistogramWidget` |
| 3.3 统一变量名 | ✅ | 确认所有文件使用 snake_case 风格 |
| 3.4 统一私有成员 | ✅ | 确认私有成员使用单下划线前缀 |

**修改文件清单**:
- 新增: `widgets/color_wheel.py`（从 `hsb_color_wheel.py` 重命名）
- 新增: `widgets/luminance_histogram_widget.py`（合并 `histogram_widget.py` 和 `luminance_histogram.py`）
- 删除: `widgets/hsb_color_wheel.py`
- 删除: `widgets/histogram_widget.py`
- 删除: `widgets/luminance_histogram.py`
- 修改: `widgets/main_window.py` - 更新导入路径
- 修改: `widgets/__init__.py` - 更新导出列表
...
**修改文件清单**:
- `color_utils.py` - 为 `rgb_to_lab()` 函数添加详细的颜色空间转换注释
- `widgets/base_canvas.py` - 为坐标转换和图片缩放算法添加详细注释

**详细改进**:

1. **color_utils.py - rgb_to_lab() 函数**
   - 添加了 LAB 颜色空间的说明
   - 详细说明了转换步骤（5个步骤）
   - 为每个步骤添加了注释说明
   - 说明了 XYZ 颜色空间转换矩阵
...
2. **widgets/base_canvas.py - 坐标转换算法**
   - `update_picker_positions()` 方法：
     - 添加了相对坐标转换算法的详细说明
     - 说明了相对坐标的优势
     - 为转换公式添加了注释
   - `on_picker_moved()` 方法：
     - 添加了画布坐标到相对坐标的转换算法说明
     - 说明了边界检查的作用
   - `canvas_to_image_pos()` 方法：
     - 添加了画布坐标到原始图片坐标的转换算法说明
     - 说明了缩放比例的计算
     - 说明了边界检查的作用

3. **widgets/base_canvas.py - 图片缩放算法**
   - `get_display_rect()` 方法：
     - 添加了保持比例缩放算法的详细说明
     - 说明了算法的5个步骤
     - 说明了算法特点（保持宽高比、居中显示等）
     - 说明了 Qt.KeepAspectRatio 的作用
...
| 任务 | 状态 | 主要工作 |
|:---|:---:|:---|
| 4.1 完善文档字符串 | ✅ | 检查确认所有文件已有完整文档字符串 |
| 4.2 添加文件头注释 | ✅ | 检查确认所有文件已有统一格式的文件头 |
| 4.3 添加关键逻辑注释 | ✅ | 为 rgb_to_lab()、坐标转换、图片缩放算法添加详细注释 |

**修改文件清单**:
- `color_utils.py` - 完善 rgb_to_lab() 函数注释
- `widgets/base_canvas.py` - 完善坐标转换和图片缩放算法注释
...
1. **已创建的基类文件**: `widgets/base_canvas.py`（非计划中的 `ui/canvases/base.py`）
   - 包含 `ImageLoader` 类：异步图片加载（使用PIL在子线程中加载）
   - 包含 `BaseCanvas` 基类（非计划中的 `BaseImageCanvas`）

2. **基类主要功能**:
   - 异步图片加载（ImageLoader线程类）
   - 图片显示（保持比例）
   - 取色点管理（使用相对坐标系统）
   - 坐标转换（画布坐标 ↔ 图片坐标）
   - 右键菜单
...
3. **已继承基类的子类**:
   - `ImageCanvas` (`widgets/image_canvas.py`): 已继承 `BaseCanvas`
   - `LuminanceCanvas` (`widgets/luminance_canvas.py`): 已继承 `BaseCanvas`

4. **实际实现与计划的差异**:
   - **基类名称**: 实际使用 `BaseCanvas` 而非 `BaseImageCanvas`
   - **文件位置**: 实际在 `widgets/base_canvas.py` 而非 `ui/canvases/base.py`
   - **实现方式**: 实际实现更加完善，包含：
     - 异步图片加载（ImageLoader线程类）
...
5. **后续调整计划**:
   - 在阶段八（目录结构重组）中，需要将 `widgets/base_canvas.py` 移动到 `ui/canvases/base.py`
   - 在阶段八中，需要将 `widgets/image_canvas.py` 移动到 `ui/canvases/image.py`
   - 在阶段八中，需要将 `widgets/luminance_canvas.py` 移动到 `ui/canvases/luminance.py`
...
1. **已创建的基类文件**: `widgets/base_card.py`（非计划中的 `ui/widgets/cards/base.py`）
   - 包含 `BaseCard` 类：单个卡片基类
   - 包含 `BaseCardPanel` 类：面板基类

2. **基类主要功能**:
   - `BaseCard`: 提供统一的卡片接口（setup_ui, clear）
   - `BaseCardPanel`: 提供卡片列表管理
   - 卡片数量控制（2-5个）
   - 批量清空卡片
   - 自动创建初始卡片

3. **已继承基类的子类**:
   - `ColorCard` (`widgets/color_card.py`): 已继承 `BaseCard`
   - `ColorCardPanel` (`widgets/color_card.py`): 已继承 `BaseCardPanel`
   - `LuminanceCard` (`widgets/luminance_card.py`): 已继承 `BaseCard`
   - `LuminanceCardPanel` (`widgets/luminance_card.py`): 已继承 `BaseCardPanel`

4. **实际实现与计划的差异**:
   - **文件位置**: 实际在 `widgets/base_card.py` 而非 `ui/widgets/cards/base.py`
   - **实现方式**: 实际实现更加简洁，专注于卡片管理功能：
     - 统一的卡片接口
     - 自动初始化卡片列表
     - 灵活的卡片创建方法
     - 更清晰的职责分离
...
6. **后续调整计划**:
   - 在阶段八（目录结构重组）中，需要将 `widgets/base_card.py` 移动到 `ui/widgets/cards/base.py`
   - 在阶段八中，需要将 `widgets/color_card.py` 移动到 `ui/widgets/cards/color.py`
   - 在阶段八中，需要将 `widgets/luminance_card.py` 移动到 `ui/widgets/cards/luminance.py`

7. **已知问题**:
   - **字体警告**: 程序启动时会出现 `QFont::setPointSize: Point size <= 0 (-1), must be greater than 0` 警告
   - **影响范围**: 不影响程序正常运行，仅控制台警告
   - **原因**: 样式表中使用 `px` 单位设置字体大小（如 `font-size: 11px`）
   - **涉及文件**: 约 20+ 处，包括 `color_card.py`, `luminance_card.py`, `luminance_histogram_widget.py`, `rgb_histogram_widget.py`, `settings_interface.py`, `update_dialog.py` 等
   - **修复方案**: 将样式表中的 `px` 单位改为 `pt` 单位（如 `font-size: 11pt`）
   - **处理计划**: 建议在阶段九（模块拆分）或后续优化阶段统一处理
...
**对比 LuminanceHistogramWidget 和 RGBHistogramWidget**:

| 功能 | LuminanceHistogramWidget | RGBHistogramWidget | 公共 |
|:---:|:---:|:---:|:---:|
| 直方图绘制 | ✓ | ✓ | ✓ |
| 数据设置 | ✓ | ✓ | ✓ |
| 清空 | ✓ | ✓ | ✓ |
...
**文件**: `widgets/base_histogram.py`

**实现内容**:
- 提取公共属性和方法
- 提供通用的数据管理（`set_data()`, `clear()`）
- 提供通用的绘制框架（`paintEvent()`）
- 定义抽象方法（`_draw_histogram()`, `_draw_custom_overlay()`, `_draw_labels()`）
- 提供辅助方法（`_draw_bottom_baseline()`, `_draw_max_label()`）

**完成状态**: ✅ 已完成
- 创建了 `widgets/base_histogram.py` (158 行)
- 提取了公共的数据管理和绘制逻辑
- 定义了清晰的抽象方法接口

---

#### 任务 7.3: 重构 LuminanceHistogramWidget

**修改**: `widgets/luminance_histogram_widget.py`
...
#### 任务 7.4: 重构 RGBHistogramWidget

**修改**: `widgets/rgb_histogram_widget.py`
...
| 任务 | 状态 | 主要工作 |
|:---|:---:|:---|
| 7.1 分析现有组件 | ✅ | 分析 LuminanceHistogramWidget 和 RGBHistogramWidget 的公共逻辑 |
| 7.2 设计基类 | ✅ | 创建 `BaseHistogram` 基类，提取公共功能 |
| 7.3 重构 LuminanceHistogramWidget | ✅ | 继承 BaseHistogram，消除 44 行重复代码 |
| 7.4 重构 RGBHistogramWidget | ✅ | 继承 BaseHistogram，消除 25 行重复代码 |
| 7.5 测试验证 | ✅ | 语法检查和功能测试全部通过 |

**新增文件**:
- `widgets/base_histogram.py` (158 行) - 直方图基类

**修改文件**:
- `widgets/luminance_histogram_widget.py` - 继承 BaseHistogram，从 353 行减少到 309 行
- `widgets/rgb_histogram_widget.py` - 继承 BaseHistogram，从 184 行减少到 159 行
- `widgets/__init__.py` - 添加 BaseHistogram 和 RGBHistogramWidget 导出
...
**后续调整计划**:
- 在阶段八（目录结构重组）中，需要将 `widgets/base_histogram.py` 移动到 `ui/widgets/histograms/base.py`
- 在阶段八中，需要将 `widgets/luminance_histogram_widget.py` 移动到 `ui/widgets/histograms/luminance.py`
- 在阶段八中，需要将 `widgets/rgb_histogram_widget.py` 移动到 `ui/widgets/histograms/rgb.py`
...
├── core/                   # 核心功能模块目录
│   ├── __init__.py        # 统一导出接口
│   ├── color.py           # 颜色处理模块，包含颜色转换、明度计算、直方图计算等功能
│   └── config.py          # 配置管理模块，负责配置的保存和加载
├── ui/                     # UI模块目录，包含界面组件和面板
│   ├── __init__.py        # 统一导出接口
...
│   ├── widgets/           # 自定义控件目录
│   │   ├── __init__.py
│   │   ├── cards.py       # 卡片组件模块（包含ColorCard、LuminanceCard等）
│   │   ├── histograms.py  # 直方图组件模块（包含LuminanceHistogramWidget、RGBHistogramWidget等）
│   │   ├── color_picker.py # 颜色选择器模块
│   │   ├── color_wheel.py # 颜色轮模块
│   │   └── zoom_viewer.py # 缩放查看器模块
│   └── interfaces/        # 界面面板目录
│       ├── __init__.py
│       ├── color_extract.py    # 颜色提取界面模块
│       ├── luminance_extract.py # 明度提取界面模块
│       └── settings.py          # 设置界面模块
├── dialogs/               # 对话框模块目录，包含各类对话框
│   ├── __init__.py        # 统一导出接口
│   ├── about_dialog.py   # 关于对话框模块，包含应用程序信息和相关链接
│   └── update_dialog.py  # 更新检查对话框模块，负责版本更新检查
├── utils/                 # 工具函数模块目录
│   ├── __init__.py        # 统一导出接口
│   ├── icon.py            # 图标工具模块
...
| `widgets/base_canvas.py` | `ui/canvases/base.py` | 移动 |
| `widgets/image_canvas.py` | `ui/canvases/image.py` | 移动 + 重命名 |
| `widgets/luminance_canvas.py` | `ui/canvases/luminance.py` | 移动 + 重命名 |
| `widgets/base_card.py` | `ui/widgets/cards.py` | 移动 + 重命名（合并） |
| `widgets/color_card.py` | 合并到 `ui/widgets/cards.py` | 合并 |
| `widgets/luminance_card.py` | 合并到 `ui/widgets/cards.py` | 合并 |
| `widgets/base_histogram.py` | `ui/widgets/histograms.py` | 移动 + 重命名（合并） |
| `widgets/luminance_histogram_widget.py` | 合并到 `ui/widgets/histograms.py` | 合并 |
| `widgets/rgb_histogram_widget.py` | 合并到 `ui/widgets/histograms.py` | 合并 |
| `widgets/color_picker.py` | `ui/widgets/color_picker.py` | 移动 |
| `widgets/color_wheel.py` | `ui/widgets/color_wheel.py` | 移动 |
...
| 原文件 | 新文件 | 操作 |
|:---|:---|:---|
| `widgets/about_dialog.py` | `dialogs/about_dialog.py` | 移动 |
| `widgets/update_dialog.py` | `dialogs/update_dialog.py` | 移动 |
...
from .widgets import ColorCardPanel
from core import get_color_info, get_config_manager
```

**ui/canvases/image.py 更新**:
```python
# 修改前
...
**导入路径修复说明**:
- 将所有三级相对导入（`...core`）改为绝对导入（`from core import ...`）
- 将 `ui/interfaces/settings.py` 中的 `from ..dialogs` 改为 `from dialogs`
- 将 `dialogs/` 中的 `from ..utils` 改为 `from utils`
- 删除 `ui/main_window.py` 中的空类定义（ColorExtractInterface、LuminanceExtractInterface）
- 在 `ui/interfaces/color_extract.py` 中添加 `Qt` 导入
- 在 `ui/interfaces/luminance_extract.py` 中添加 `Qt` 导入
- 确保所有模块都使用正确的导入方式
...
| 任务 | 状态 | 主要工作 |
|:---|:---:|:---|
| 8.1 创建目录结构 | ✅ | 创建core、ui、dialogs、utils等目录 |
| 8.2 迁移核心模块 | ✅ | 迁移color_utils.py、config_manager.py到core/ |
| 8.3 迁移工具模块 | ✅ | 拆分icon_utils.py到utils/icon.py和utils/platform.py |
| 8.4 迁移UI模块 | ✅ | 迁移widgets/到ui/，合并同类模块 |
| 8.5 迁移对话框模块 | ✅ | 迁移about_dialog.py、update_dialog.py到dialogs/ |
| 8.6 更新导入路径 | ✅ | 更新所有文件的导入路径 |
| 8.7 全面测试 | ✅ | 语法检查和功能测试全部通过 |

**新增目录**:
- `core/` - 核心功能模块
- `ui/` - UI模块（包含canvases、widgets、interfaces子目录）
- `dialogs/` - 对话框模块
- `utils/` - 工具函数模块
- `logo/` - Logo资源目录
- `file/` - 文件资源目录
...
**删除目录**:
- `widgets/` - 已迁移到ui/下的相应子目录

**移动文件**:
- `color_utils.py` → `core/color.py`
- `config_manager.py` → `core/config.py`
- `icon_utils.py` → `utils/icon.py` 和 `utils/platform.py`
- `widgets/main_window.py` → `ui/main_window.py`
- `widgets/base_canvas.py` → `ui/canvases/base.py`
- `widgets/image_canvas.py` → `ui/canvases/image.py`
- `widgets/luminance_canvas.py` → `ui/canvases/luminance.py`
- `widgets/base_card.py`、`widgets/color_card.py`、`widgets/luminance_card.py` → `ui/widgets/cards.py`
- `widgets/base_histogram.py`、`widgets/luminance_histogram_widget.py`、`widgets/rgb_histogram_widget.py` → `ui/widgets/histograms.py`
- `widgets/color_picker.py` → `ui/widgets/color_picker.py`
- `widgets/color_wheel.py` → `ui/widgets/color_wheel.py`
- `widgets/zoom_viewer.py` → `ui/widgets/zoom_viewer.py`
- `widgets/settings_interface.py` → `ui/interfaces/settings.py`
- `widgets/about_dialog.py` → `dialogs/about_dialog.py`
- `widgets/update_dialog.py` → `dialogs/update_dialog.py`

**保留文件**:
- `version.py` - 保留在根目录

**合并说明**:
- 将同类模块合并到同一个文件中，如所有卡片相关类合并到`ui/widgets/cards.py`，所有直方图相关类合并到`ui/widgets/histograms.py`
- 保持模块化，但避免过度拆分，提高代码可维护性
- 将界面面板拆分到独立的文件中（color_extract.py、luminance_extract.py）

**参考规范**:
- 参考 BetterGI StellTrack 开发规范中的项目结构
- 按功能模块化组织代码
- 同类模块整合到一块，避免过度拆分
- 保持清晰的目录层次结构
...
**目标**: 将当前多级嵌套结构简化为2级结构，合并功能相近的模块，实现模块化与结构清晰的平衡。
...
#### 合并步骤 1: 合并画布模块到 ui 根目录 ✅ 已完成

**合并理由**: `canvases/` 目录下只有3个文件（base.py、image.py、luminance.py），且都是画布相关，可以合并到 `ui/canvases.py` 单一文件中。

**操作**:
| 原文件 | 新文件 | 操作 | 状态 |
|:---|:---|:---|:---:|
| `ui/canvases/base.py` | `ui/canvases.py` | 合并 | ✅ |
| `ui/canvases/image.py` | `ui/canvases.py` | 合并 | ✅ |
| `ui/canvases/luminance.py` | `ui/canvases.py` | 合并 | ✅ |

**完成效果**:
- ✅ 删除 `ui/canvases/` 目录
- ✅ 创建 `ui/canvases.py`（1127 行）
- ✅ 保持所有类名和接口不变
- ✅ 语法检查通过
- ✅ 导入路径正常工作
...
#### 合并步骤 2: 合并控件模块到 ui 根目录 ✅ 已完成

**合并理由**: `widgets/` 目录下文件较多，但可以分为两类：
1. 卡片类（cards.py）和直方图类（histograms.py）- 已有基类，保持合并状态
2. 其他独立控件（color_picker.py、color_wheel.py、zoom_viewer.py）- 保持独立

**操作**:
| 原文件 | 新文件 | 操作 | 状态 |
|:---|:---|:---|:---:|
| `ui/widgets/cards.py` | `ui/cards.py` | 移动 | ✅ |
| `ui/widgets/histograms.py` | `ui/histograms.py` | 移动 | ✅ |
| `ui/widgets/color_picker.py` | `ui/color_picker.py` | 移动 | ✅ |
| `ui/widgets/color_wheel.py` | `ui/color_wheel.py` | 移动 | ✅ |
| `ui/widgets/zoom_viewer.py` | `ui/zoom_viewer.py` | 移动 | ✅ |
| `ui/widgets/__init__.py` | 删除 | 删除 | ✅ |

**导入路径更新**:
- `ui/__init__.py` - 更新为直接从 `.cards`、`.histograms` 等导入
- `ui/main_window.py` - 更新导入路径
- `ui/canvases.py` - 更新从 `.color_picker` 和 `.zoom_viewer` 导入
- `ui/interfaces/color_extract.py` - 更新导入路径
- `ui/interfaces/luminance_extract.py` - 更新导入路径
...
#### 合并步骤 3: 合并界面模块到 ui 根目录 ✅ 已完成

**合并理由**: `interfaces/` 目录下只有3个文件（color_extract.py、luminance_extract.py、settings.py），可以合并到 `ui/interfaces.py` 单一文件中。

**操作**:
| 原文件 | 新文件 | 操作 | 状态 |
|:---|:---|:---|:---:|
| `ui/interfaces/color_extract.py` | `ui/interfaces.py` | 合并 | ✅ |
| `ui/interfaces/luminance_extract.py` | `ui/interfaces.py` | 合并 | ✅ |
| `ui/interfaces/settings.py` | `ui/interfaces.py` | 合并 | ✅ |

**完成效果**:
- ✅ 删除 `ui/interfaces/` 目录（待用户手动删除）
- ✅ 创建 `ui/interfaces.py`（592 行）
- ✅ 保持所有类名和接口不变
- ✅ 语法检查通过
- ✅ 导入测试通过
...
│   ├── __init__.py        # 统一导出接口
│   ├── main_window.py     # 主窗口类
│   ├── canvases.py        # 画布模块（合并后）
│   ├── cards.py           # 卡片组件模块
│   ├── histograms.py      # 直方图组件模块
│   ├── color_picker.py    # 颜色选择器模块
│   ├── color_wheel.py     # 颜色轮模块
│   ├── zoom_viewer.py     # 缩放查看器模块
│   └── interfaces.py      # 界面面板模块（合并后）
├── dialogs/               # 对话框模块目录
│   ├── __init__.py
│   ├── about_dialog.py
│   └── update_dialog.py
...
**结构优势**:
1. **2级结构清晰**: 所有模块最多2级目录，易于导航
2. **功能模块化**: 按功能分类（core、ui、dialogs、utils），职责明确
3. **合并合理**: 将紧密相关的类合并到同一文件，减少文件数量
4. **维护简单**: 文件位置直观，导入路径简洁
