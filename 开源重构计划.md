...
def load_image(self, path):
    print(f"Loading image: {path}")  # 调试代码
    pixmap = QPixmap(path)
    print(f"Image size: {pixmap.size()}")  # 调试代码
    return pixmap
...
def convert_relative_to_canvas(rel_x: float, rel_y: float, 
                                display_rect: tuple) -> tuple:
    """相对坐标转换为画布坐标"""
...
**优先级文件**:
1. `color_utils.py` - 所有颜色转换函数
2. `config_manager.py` - 配置管理类
3. `version.py` - 版本管理类
