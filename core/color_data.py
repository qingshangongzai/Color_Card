...
# Nice Color Palettes 配色方案数据 (500组)
...
def get_nice_palette_count():
    ...
    return len(NICE_COLOR_PALETTES_DATA)
...
def get_nice_palette(index):
    ...
    Returns:
        list: 包含5个HEX颜色值的列表
    """
    if 0 <= index < len(NICE_COLOR_PALETTES_DATA):
        return NICE_COLOR_PALETTES_DATA[index]
    return []
...
def get_random_nice_palette():
    ...
    """
    import random
    index = random.randint(0, len(NICE_COLOR_PALETTES_DATA) - 1)
    return index, NICE_COLOR_PALETTES_DATA[index]
...
def get_nice_palettes_batch(start_index=0, count=10):
    ...
    """
    end_index = min(start_index + count, len(NICE_COLOR_PALETTES_DATA))
    return [(i, NICE_COLOR_PALETTES_DATA[i]) for i in range(start_index, end_index)]
...
def get_tailwind_color_series_names():
    ...
    return list(TAILWIND_COLOR_DATA.keys())
...
def get_tailwind_color_series(series_name):
    ...
    Returns:
        dict: 颜色系列数据，包含 name, name_en, colors
    """
    return TAILWIND_COLOR_DATA.get(series_name, None)
...
def get_tailwind_light_shades(series_name):
    ...
    Returns:
        list: 5个浅色色值列表
    """
    series = TAILWIND_COLOR_DATA.get(series_name)
    if not series:
        return []
    return [series["colors"][i] for i in range(5)]
...
def get_colorbrewer_series_names():
    ...
    return list(COLORBREWER_DATA.keys())
...
def get_colorbrewer_color_series(series_name):
    ...
    Returns:
        dict: 颜色系列数据，包含 name, name_en, colors
    """
    return COLORBREWER_DATA.get(series_name, None)
