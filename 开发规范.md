...
└── widgets/
    ...
    ├── hsb_color_wheel.py   # HSB色环组件
    ├── rgb_histogram_widget.py # RGB直方图组件
...
默认情况下，导航栏会根据窗口宽度自动切换展开/折叠状态。如需固定为展开状态（始终显示图标+文字）：
...
**重要注意事项：**
1. **调用顺序**：必须先调用 `setExpandWidth()` 设置展开宽度，再调用 `setCollapsible(False)`
   - 如果顺序颠倒，`setCollapsible(False)` 会立即展开导航栏，但此时使用的是默认宽度 322px
   - 导致第一次打开时导航栏宽度过宽，影响其他组件布局

2. **展开宽度设置**：
   - 默认展开宽度为 322px，可能过宽
   - 推荐设置为 200px 左右，根据实际内容调整
   - 最小不能小于 42px

3. **相关 API：**
   - `setCollapsible(bool)`：设置导航栏是否可折叠
   - `setExpandWidth(int)`：设置展开后的宽度
   - `setMinimumExpandWidth(int)`：设置允许展开的最小窗口宽度阈值

### 4.1.3 Windows 任务栏图标显示规范
...
- 使用比例计算确保精度
- 边界检查防止越界
- 取色点位置限制在图片显示区域内（通过 `get_image_display_rect()` 方法）
    ...
    if self._dragging and self.parent():
        ...
        if hasattr(parent, 'get_image_display_rect'):
            ...
            if display_rect:
                ...
                # 限制取色点中心在图片显示区域内
                center_x = max(disp_x, min(new_pos.x() + self.radius, disp_x + disp_w))
                center_y = max(disp_y, min(new_pos.y() + self.radius, disp_y + disp_h))
...
**核心原则：**
采样点位置使用**图片相对坐标**（归一化坐标 0.0-1.0）存储，确保窗口布局变化时采样点始终保持在图片的相对位置。

**坐标系统说明：**

| 坐标类型 | 说明 | 范围 | 存储位置 |
|:---:|:---|:---:|:---:|
| 相对坐标 | 相对于图片显示区域的归一化坐标 | 0.0-1.0 | `_picker_rel_positions` |
| 画布坐标 | 相对于画布控件的像素坐标 | 像素值 | `_picker_positions` |
...
1. **存储结构**
```python
# 使用 QPointF 存储相对坐标（浮点数）
self._picker_rel_positions = []  # 存储相对于图片的归一化坐标 (0.0-1.0)
self._picker_positions = []      # 存储画布坐标（用于显示）
```

2. **初始化默认位置**
```python
# 默认在图片中心
self._picker_rel_positions.append(QPointF(0.5, 0.5))

# 多个采样点时，在中心区域均匀分布
offset_x = (i - 2) * 0.05  # 5% 的相对偏移
self._picker_rel_positions[i] = QPointF(0.5 + offset_x, 0.5)
```
...
def resizeEvent(self, event):
    ...
    if self._image and not self._image.isNull():
        ...
        self.update_picker_positions()  # 关键：重新计算画布坐标
        self.extract_all_colors()
        self.update()
```

**注意事项：**
- 相对坐标使用 `QPointF`（浮点数），画布坐标使用 `QPoint`（整数）
- 边界限制：相对坐标必须限制在 [0.0, 1.0] 范围内
- 清空图片时，重置相对坐标到默认位置（0.5, 0.5）
- 动态调整采样点数量时，同步维护 `_picker_rel_positions` 列表
...
- 鼠标按下：显示抓取光标，高亮显示
- 拖动时：实时更新颜色值
- 边界限制：限制在图片显示区域内（通过 `get_image_display_rect()` 获取图片显示区域并限制取色点中心在该区域内）
...
| `settings.color_modes` | list | ["HSB", "LAB"] | 色卡中显示的两种色彩模式 |
| `settings.color_sample_count` | int | 5 | 色彩提取采样点数量（2-5） |
| `settings.luminance_sample_count` | int | 5 | 明度提取采样点数量（2-5） |
...
**功能说明：**
用户可以通过设置面板分别调整色彩提取和明度提取的采样点数量，范围2-5。

**实现要点：**

1. **配置管理**
   - 使用 `settings.color_sample_count` 存储色彩提取采样点数
   - 使用 `settings.luminance_sample_count` 存储明度提取采样点数

2. **设置界面**
   - 使用 `ComboBox` 下拉列表提供选项（2、3、4、5）
   - 发射 `color_sample_count_changed` 和 `luminance_sample_count_changed` 信号

3. **画布组件**
   - 实现 `set_picker_count(count)` 方法动态调整取色点数量
   - 增加取色点时，如果有图片则立即显示
   - 调整数量后自动重新提取颜色/明度

4. **卡片面板**
   - 实现 `set_card_count(count)` 方法动态调整卡片数量
   - 新增卡片自动应用当前显示设置
...
| 版本 | 日期 | 变更内容 |
|:---:|:---:|:---:|
| 4.4 | 2026-02-04 | 新增采样点坐标管理规范（6.2.1节）：采样点位置使用图片相对坐标（归一化坐标 0.0-1.0）存储，确保窗口布局变化时采样点始终保持在图片的相对位置；更新 `image_canvas.py` 和 `luminance_canvas.py` 实现相对坐标系统 |
| 4.3 | 2026-02-04 | 新增导航栏显示模式规范（4.1.2节）：添加导航栏始终展开的配置说明和注意事项；强调 `setExpandWidth()` 必须在 `setCollapsible(False)` 之前调用；更新 `main_window.py` 实现导航栏始终展开 |
| 4.2 | 2026-02-04 | 新增HSB色环和RGB直方图功能：在色彩提取面板右侧添加HSB色环（显示采样点在HSB色彩空间中的位置）和RGB直方图（显示图片RGB三通道分布）；添加 `hsb_color_wheel.py` 和 `rgb_histogram_widget.py` 组件；更新 `color_utils.py` 添加RGB直方图计算函数；使用QPixmap缓存优化HSB色环绘制性能 |
| 4.1 | 2026-02-04 | 修复取色点可拖动到图片显示区域外的问题：添加 `get_image_display_rect()` 方法供子组件查询图片显示区域；更新 `color_picker.py` 限制取色点拖动范围在图片显示区域内；更新交互设计规范（8.1节）和坐标映射规范（6.2节） |
| 4.0 | 2026-02-04 | 新增动态采样点调节功能：支持在设置面板分别调整色彩提取和明度提取的采样点数量（2-5）；添加采样点数量配置规范；更新 `config_manager.py`、`settings_interface.py`、`image_canvas.py`、`luminance_canvas.py`、`color_card.py`、`luminance_card.py`、`main_window.py` 实现动态采样点调节 |
