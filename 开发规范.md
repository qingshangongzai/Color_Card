...
Color Extractor 是一个基于 PySide6 开发的 GUI 程序，用于从图片中提取颜色，显示 HSB 和 LAB 颜色值，以及明度分析。核心功能包括：

- 图片导入（JPG、PNG、BMP、GIF）
- **色彩提取**：5个可拖动取色点，实时显示 HSB 和 LAB 颜色值
...
├── color_utils.py           # 颜色转换工具（RGB ↔ HSB/LAB/明度）
├── README.md                # 项目文档
├── 开发规范.md              # 本文件
└── widgets/
    ├── __init__.py          # 统一导出接口
...
- 使用 Rec. 709 标准计算亮度值
- 将 0-255 的明度值分为8个区域（Zone 0-1 到 Zone 7-8）
- 直方图计算使用采样优化，提高性能
...
设置面板使用 `ScrollArea` 作为基类，支持滚动显示多个设置分组。
...
```python
from qfluentwidgets import ScrollArea, SettingCardGroup, PushSettingCard, FluentIcon
...
关于对话框使用 `QDialog` 作为基类，使用 `QTextBrowser` 显示富文本内容。

**规范要求：**
- 使用 `QTextBrowser` 显示 HTML 内容
- 支持超链接点击 (`setOpenExternalLinks(True)`)
