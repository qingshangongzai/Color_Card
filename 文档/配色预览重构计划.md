...
1. **创建 `scene_types.json`** - 场景类型定义（只读）
   - 定义所有场景类型
   - `custom` 标记为 `builtin: false`

2. **创建各场景的 `layout.json`** - 布局配置（只读）
   - 为每个内置场景创建 layout.json
   - 仅包含布局类型和参数
   - `custom` 无需 layout.json，固定使用 `single` 布局

3. **扩展 `ConfigManager`** - 支持用户模板索引
...
scene_config = config_loader.load("ui")  # 加载 index.json
...
1. **数据结构准备**
   - 创建 `scene_types.json` 场景类型定义
   - 为每个内置场景创建 `layout.json` 布局配置
   - 扩展 `ConfigManager` 支持 `scene_templates` 字段

2. **代码重构**
   - 实现 `SceneTypeManager` 类 - 加载场景类型
   - 实现 `LayoutLoader` 类 - 加载 layout.json
   - 实现 `LayoutFactory` 和各布局类
   - 扩展 `ConfigManager` - 用户模板索引管理
   - 修改 `ColorPreviewInterface` - 使用布局工厂
