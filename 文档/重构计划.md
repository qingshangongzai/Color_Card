...
**验收标准**：
- [ ] 图片状态统一由中介者管理
- [ ] 消除 main_window 中的 `_is_clearing` 标志
- [ ] ImageCanvas 和 LuminanceCanvas 通过中介者同步
...
**当前文件**：`ui/preview_widgets.py`（约1100行）

**整合后结构**：
```
ui/color_preview.py
├── DraggableColorDot          # 颜色圆点组件
├── ColorDotBar                # 颜色圆点工具栏
├── BasePreviewScene           # 预览场景基类
├── PreviewSceneFactory        # 预览场景工厂
├── SVGPreviewWidget           # SVG预览组件
├── BaseLayout                 # 布局基类
├── SingleLayout               # 单图布局
├── ScrollLayout               # 滚动布局
├── GridLayout                 # 网格布局
├── MixedLayout                # 混合布局
├── SceneTypeSelector          # 场景类型选择器
└── ColorPreviewInterface      # 预览界面
```

**验收标准**：
- [ ] 所有预览组件在同一文件
- [ ] 代码按功能区域用注释分隔
- [ ] 删除原 preview_widgets.py
...
**重构方案**：
```python
def _generate_highlight_pixmap(self, mode: str, threshold: int) -> QPixmap:
    """
    mode: 'saturation' 或 'brightness'
    """
    ...

def _draw_highlight(self, mode: str, threshold: int) -> None:
    ...
```
