...
1. **新建** `core/image_memory_manager.py` 模块：
   - `ImageMemoryManager` 类：使用 OrderedDict 实现 LRU 缓存
   - 内存限制（默认 500MB），自动清理最久未使用的图片
   - 统计功能：缓存命中率、内存占用、清理次数
   - 全局实例 `get_memory_manager()`
2. **修改** `core/image_service.py`：
   - 集成 `ImageMemoryManager`
   - 加载图片时先检查缓存
   - 加载完成后添加到内存管理器
   - 新增 `release_current_image()` 方法释放内存
   - 新增 `get_memory_stats()` 获取统计信息
   - 优化 `generate_thumbnail()` 使用 QImage 进行缩放
3. **修改** `ui/canvases.py`：
   - 优化 `BaseCanvas.clear_image()`：显式调用 `detach()` 释放 Qt 对象，触发垃圾回收
   - 优化 `ImageCanvas.clear_image()`：清理高亮遮罩缓存
   - 优化 `LuminanceCanvas.clear_image()`：清理 Zone 高亮缓存
4. **更新** `core/__init__.py`：导出 `ImageMemoryManager`、`get_memory_manager`、`set_memory_limit`
5. **更新** `文档/开发规范.md`：添加第 7.4 节内存管理规范
...
### 任务 5.1：移动 theme_colors.py ✅

**目标**：将 `ui/theme_colors.py` 移至 `utils/`

**原因**：纯工具函数（283行），归入 utils 更合理，无需额外创建 ThemeService

**涉及文件**：

- `ui/theme_colors.py` → `utils/theme_colors.py`
- 所有导入 `from ui.theme_colors import ...` 的文件
...
- ✅ `ImageService` 内部延迟初始化 `_memory_manager`
- ✅ 添加 `_get_memory_manager()` 方法，首次使用时才获取内存管理器

**阶段二：ServiceFactory 服务工厂**

- ✅ 新建 `core/service_factory.py` 模块
- ✅ `ServiceFactory` 类实现延迟创建、单例模式、线程安全（双重检查锁定）
- ✅ 提供 `get_color_service()`、`get_palette_service()`、`get_image_service()`、`get_luminance_service()` 方法
- ✅ 更新 `core/__init__.py` 导出 `ServiceFactory`
- ✅ 改造 `ColorExtractInterface` 使用 `ServiceFactory` 延迟获取 `ColorService`
- ✅ 改造 `PaletteManagementInterface` 使用 `ServiceFactory` 延迟获取 `PaletteService`
- ✅ 改造 `BaseCanvas` 使用 `ServiceFactory` 延迟获取 `ImageService`
- ✅ 改造 `LuminanceCanvas` 使用 `ServiceFactory` 延迟获取 `LuminanceService`
...
│   ├── service_factory.py    # 新增：服务工厂（单例化、延迟创建）
│
├── utils/                     # 纯工具（公共模块）
│   ├── __init__.py
│   ├── icon.py
