...
#### 阶段 2.5：替换原目录 ⏸️ 暂停（等待阶段3.x）

**目标**：验证通过后，用新格式文件替换原文件

**状态**：转换后的文件已保存在 `color_data_new/` 目录，暂不替换原目录。原因：当前加载代码不兼容新格式，需要先完成阶段3.x（加载函数重构）。

**步骤**：

1. ⏸️ 备份原 `color_data/` 目录为 `color_data_backup/`
2. ⏸️ 将 `color_data_new/` 中的文件复制到 `color_data/`
3. ⏸️ 验证应用加载正常
4. ⏸️ 确认无误后删除 `color_data_backup/` 和 `color_data_new/`
...
```
┌─────────────────┐
│  color_data/    │ ← 原目录（应用当前使用）
└────────┬────────┘
         │ 复制
         ▼
┌─────────────────┐
│ color_data_new/ │ ← 副本目录（转换工作区）
└────────┬────────┘
         │ 运行转换脚本
         ▼
┌─────────────────┐
...
**目标**：简化 `core/color_data.py`，消除重复代码，支持JSON内部分组
...
2. 实现注册、加载、查询等基础方法
3. 创建 `ColorSource` 类，封装分组读取逻辑
4. 暂不修改现有函数

**ColorSource类设计（已简化，统一为palettes格式）**：

```python
class ColorSource:
    """统一配色源接口（所有配色源统一为palettes格式）"""
    ...
    def get_palettes_for_group_batch(self, group_index: int, start: int, count: int) -> list:
        ...
        if group_index >= len(self._groups):
            return []
        indices = self._groups[group_index].get("indices", [])
        batch_indices = indices[start:start + count]
        return [self._palettes[i] for i in batch_indices if i < len(self._palettes)]
```

**验证点**：

- 注册表类可正常工作
- `ColorSource` 类可正确读取分组
- 不影响现有功能
...
2. 创建 `get_groups()` 函数（读取JSON内部分组）
3. 创建 `get_palettes_for_group()` 函数（按分组获取配色）
...
2. **移除** 所有 `_load_xxx_series()` 方法（被统一接口替代）
3. 测试所有配色源切换
...
### 阶段 5.x：色彩管理面板分组功能（后续版本）

**目标**：为色彩管理面板添加分组功能，与统一格式配合

**前置条件**：阶段 4.x 完成（UI层统一接口已实现）
