...
|**当前版本** |阶段 4.x |UI层简化 |阶段 3.x 完成 | ✅ 已完成
...
- 在 `core/color_data.py` 的 `ColorSource` 类中添加 `get_palettes_for_group_batch()` 方法
- 在 `ui/preset_color_widgets.py` 中创建 `GroupLoaderThread` 线程类
- 在 `PresetColorList` 类中添加分批加载逻辑：
  - `BATCH_THRESHOLD = 20`：分组内配色超过 20 组时启用分批加载
  - `BATCH_SIZE = 10`：每批加载 10 组配色
  - 实现了 `_cancel_loader()`、`_start_batch_loading()`、`_on_batch_data_ready()`、`_on_loading_finished()` 方法
