...
**核心问题**：Nuitka 与 PyInstaller 有两个关键运行时差异：

| 差异 | PyInstaller | Nuitka | 后果 |
|------|------------|--------|------|
| 打包标志 | `sys.frozen = True` | ❌ 不设置 | 程序误判为开发环境，跳过文件复制 |
| 可执行文件路径 | `sys.executable` = exe | ❌ 指向 Python 解释器 | `[WinError 2]` 找不到文件 |
...
### 16.3 正确方案：InfoPlist.strings 本地化
