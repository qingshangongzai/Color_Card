...
**关键要点：**
- 每行独立的 QHBoxLayout 可以独立控制拉伸
- 使用 `addWidget(card, stretch=1)` 让色卡均匀分布并占满整行
- 清空时需要递归删除行布局中的色卡
...
|布局方式 |优点 |缺点 |适用场景 |
|:---:|:---|:---|:---|
|QGridLayout |结构简单，行列对齐 |所有行共享列宽，无法独立拉伸 |每行列数相同 |
|QVBoxLayout + QHBoxLayout |每行独立拉伸，可自适应 |代码稍复杂 |每行列数不同 |
