...
|`settings.language` |string |"zh_CN" |界面语言（zh_CN/en_US） |
...
|版本 |日期 |变更内容 |
|:---:|:---:|:---:|
|3.23 |2026-02-19 |新增多语言国际化功能：新建 `utils/locale.py` 模块，提供 `LocaleManager`、`tr()`、`set_language()` 等功能；创建 `locales/` 目录存放语言包（zh_CN.json、en_US.json）；所有界面和对话框支持国际化；设置界面添加语言切换选项；支持语言切换后动态更新界面；更新项目结构和开发规范（新增第15章多语言国际化规范） |
