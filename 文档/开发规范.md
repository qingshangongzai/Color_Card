...
**适用条件**：SVG有语义化类型信息（通过class/id关键词识别）
...
|`settings.language` |string |"ZW_JT" |界面语言（ZW_JT/ZW_FT/EN_US） |
...
├── screenshots/               # 软件截图
│   ├── color-extract.png
│   ├── color-generation.png
│   ├── color-preview.png
│   ├── luminance-extract.png
│   ├── palette-management.png
│   └── preset-colors.png
...
- 发布新版本时同步更新 `changelog.json`
- 保持两个文件同步（docs 和 website/public）
- 使用数组格式存储更新内容
- 日期格式：YYYY-MM-DD
...
- 宽度：1200-1920px
- 格式：PNG
- 命名：使用小写英文字母，如 `color-extract.png`

**更新步骤：**

1. 替换 `website/public/screenshots/` 中的图片
2. 同步复制到 `docs/screenshots/`
3. 刷新网页查看效果

### 15.4 部署流程

**自动部署：**
- 推送代码到 GitHub 后自动触发 GitHub Actions 部署
- 部署源：`docs/` 目录
...
# 同步配色数据（如有更新）
Copy-Item color_data\* docs\palettes\ -Force
```
...
class MyInterface(QWidget):
    ...
    def update_texts(self):
        ...
        self.button.setText(tr('my_interface.button'))
```
...
| 对话框 | `dialogs.{dialog}.*` | `dialogs.about.title` |
| 通用文本 | `common.*` | `common.delete` |
...
**文件命名：**
- 格式：`Color Card_版本号_YYYYMMDD_HHMMSS.log`
- 示例：`Color Card_1.4.0_20260302_143025.log`
- 每次启动应用创建一个新的日志文件
...
pytest-qt 提供了 `qtbot` fixture 用于 Qt 组件测试：
...
```python
def test_safe_cancel_without_terminate(qtbot):
...
# 运行特定测试
python -m pytest tests/test_threading.py::TestHistogramCalculator::test_cancel_does_not_block
...
5. **使用 fixture 复用代码**：提取公共设置代码到 fixture
6. **Mock 外部依赖**：使用 `unittest.mock` 隔离外部依赖
...
|版本 |日期 |变更内容 |
|:---:|:---:|:---:|
|3.36 |2026-03-02 |优化日志系统：每次启动创建新日志文件，文件名格式为 `Color Card_版本号_YYYYMMDD_HHMMSS.log`；移除按大小轮转策略，改为按启动创建新文件；新增日志清理策略，自动清理30天前的日志文件；更新第17.2节日志配置说明 |
|3.35 |2026-02-28 |日志系统第二阶段：核心模块日志接入（color_service.py、image_service.py、config.py）；用户操作日志接入（color_extract.py、canvases.py）；添加性能日志记录（extract_dominant_colors、load_image） |
|3.34 |2026-02-28 |新增日志系统：创建 `core/logger.py` 模块，包含 `LoggerManager`、`get_logger`、`log_user_action`、`log_performance`；日志存储在 `~/.color_card/logs/` 目录，支持文件轮转（最大10MB，保留7个备份）；在 `main.py` 中集成日志初始化和全局异常捕获；更新项目结构和开发规范（新增第17章日志系统规范） |
|3.33 |2026-02-28 |P3多线程优化修复：优化 `HistogramCalculator` 和 `ProgressiveImageLoader` 的 `cancel()` 方法移除 `wait()` 调用；为所有线程类和服务类添加线程安全文档注释；创建 `tests/` 测试目录，包含 19 个多线程测试用例；创建 `pytest.ini` 配置文件和 `tests/requirements-dev.txt` 开发依赖文件；新增第20章测试规范；更新章节编号（原第20章开源许可证管理规范改为第21章，原第21章附录改为第22章） |
|3.32 |2026-02-27 |重构明度分析Zone分区：将直方图分区从8个改为9个（Zone 0-8），使用 `ZONE_WIDTH = 255/9 ≈ 28.33` 计算Zone边界；更新Zone标签命名（黑色×1、阴影×2、中间调×3、高光×2、白色×1）；修改 `core/color.py`、`ui/histograms.py`、`ui/canvases.py`、`core/luminance_service.py` 中的Zone计算逻辑；更新开发规范第6.3节Zone分区规范；更新README.md明度分析描述 |
|3.31 |2026-02-26 |优化SVG智能映射逻辑：新增覆盖检测功能，检测被完全覆盖的元素并跳过映射；简化配色分配逻辑，所有元素按面积平等处理；根据面积最大元素的覆盖率决定是否添加背景矩形（<90%添加，>=90%不添加）；更新第9.7.3节智能映射规则说明 |
|3.30 |2026-02-26 |新增渐变提取功能：创建 `core/gradient.py` 模块实现三种颜色空间（RGB/HSB/LAB）的渐变插值算法；创建 `ui/gradient_extract.py` 模块实现渐变提取界面（GradientExtractInterface、GradientCardPanel、GradientPreviewWidget）；支持起始/结束颜色选择、中间色数量控制、渐变预览、随机生成、收藏功能；收藏时先打开编辑配色对话框；更新项目结构、开发规范和语言包 |
|3.29 |2026-02-24 |新增项目官网：创建 `docs/` 和 `website/` 目录，添加官网页面（功能特性、界面预览、配色方案、更新日志、下载），支持 GitHub Pages 自动部署；新增第15章项目官网维护规范；更新 README.md 添加官网链接；更新关于对话框添加官网按钮 |
|3.28 |2026-02-22 |类型注解规范化：修复所有使用小写内置类型（`list`、`dict`、`tuple`、`type`）的问题，统一使用 `typing` 模块（`List`、`Dict`、`Tuple`、`Type`）；移除 Python 3.9+ 内置泛型语法（如 `list[str]`）；涉及 core/ 和 ui/ 目录下 15 个文件；新增第 3.7 节类型注解规范 |
|3.27 |2026-02-22 |业务服务延迟加载优化：新建 `core/service_factory.py` 模块，实现服务单例化和延迟创建（ServiceFactory）；修改 `ImageService` 内部延迟初始化 `_memory_manager`；改造 `ColorExtractInterface`、`PaletteManagementInterface`、`BaseCanvas`、`LuminanceCanvas` 使用 ServiceFactory 延迟获取服务；更新 `core/__init__.py` 导出 ServiceFactory；清理冗余代码；更新项目结构和开发规范 |
|3.26 |2026-02-21 |移动 theme_colors.py 到 utils/：将 `ui/theme_colors.py` 移至 `utils/theme_colors.py`，更新所有代码文件的导入路径（`from ui.theme_colors` → `from utils.theme_colors`）；更新项目结构和开发规范；更新重构计划（任务5.1完成，删除原任务5.4） |
|3.25 |2026-02-20 |提取公共基类 `BaseCache`，新建 `core/cache_base.py` 模块，包含LRU机制、统计功能等通用缓存功能；重构 `ColorSchemeCache` 和 `HistogramCache` 继承 `BaseCache`，消除约100行重复代码；删除 `histogram_cache.py` 中未使用的 `Dict` 导入；更新 `core/__init__.py` 导出 `BaseCache`；更新项目结构和开发规范 |
|3.24 |2026-02-20 |新建 `core/color_scheme_cache.py` 模块，包含 `ColorSchemeCache` 类，实现配色计算结果缓存（LRU策略，最大100条，色相精度0.1度）；新建 `core/histogram_cache.py` 模块，包含 `HistogramCache` 类和 `ImageFingerprintGenerator`，实现直方图数据缓存（LRU策略，最大50条，图片指纹基于尺寸和9点采样）；修改 `core/color.py`，`get_scheme_preview_colors` 和 `get_scheme_preview_colors_ryb` 函数添加 `use_cache` 参数并集成缓存；修改 `core/histogram_service.py`，`HistogramService` 类添加 `use_cache` 参数，明度/RGB/色相直方图计算均集成缓存；更新 `core/__init__.py` 导出缓存相关类和函数；更新项目结构、开发规范和重构计划 |
|3.23 |2026-02-19 |新增多语言国际化功能：新建 `utils/locale.py` 模块，提供 `LocaleManager`、`tr()`、`set_language()` 等功能；创建 `locales/` 目录存放语言包（ZW_JT.json、ZW_FT.json、EN_US.json）；所有界面和对话框支持国际化；设置界面添加语言切换选项；支持语言切换后动态更新界面；更新项目结构和开发规范（新增第15章多语言国际化规范） |
|3.22 |2026-02-18 |创建 HistogramService 并优化直方图计算：新建 `core/histogram_service.py` 模块，将直方图计算逻辑从UI层迁移到业务层；`HistogramCalculator` 在后台线程执行计算，支持明度、RGB、色相直方图；`HistogramService` 管理计算任务生命周期，支持延迟计算（500ms）和取消机制；大图片自动使用更大采样步长优化性能；`LuminanceHistogramWidget`、`RGBHistogramWidget`、`HueHistogramWidget` 使用异步计算；清理冗余代码（删除 `HueHistogramWidget._calculate_hue_histogram()` 方法，删除未使用的 `colorsys`、`calculate_histogram`、`calculate_rgb_histogram` 导入）；更新项目结构、开发规范和重构计划 |
|3.21 |2026-02-18 |创建 PreviewService 并进一步下沉业务逻辑：新建 `core/preview_service.py` 模块，将预览场景管理逻辑从 `ui/color_preview.py` 下沉到业务层；`PreviewService` 管理场景配置加载、模板管理、SVG配色映射、文件操作；`ColorPreviewInterface` 和 `SVGPreviewWidget` 只保留UI逻辑；清理未使用的导入（`datetime`, `Path`）；更新项目结构、开发规范和重构计划 |
|3.20 |2026-02-18 |创建 ImageService 和 LuminanceService：新建 `core/image_service.py` 模块，将 `ProgressiveImageLoader` 从 `ui/canvases.py` 迁移到业务层，`ImageService` 管理图片加载任务生命周期；新建 `core/luminance_service.py` 模块，将明度计算、Zone分析、高亮区域计算下沉到业务层；`BaseCanvas` 和 `LuminanceExtractInterface` 使用服务层替代直接线程管理；清理未使用的导入（`io`, `PIL.Image`, `QThread`）；更新项目结构和开发规范 |
|3.19 |2026-02-18 |创建 PaletteService：新建 `core/palette_service.py` 模块，将配色导入导出逻辑从 `PaletteManagementInterface` 和 `ConfigManager` 迁移到业务层；`PaletteManagementInterface` 只保留UI逻辑；简化 `ConfigManager` 移除 `import_favorites()` 和 `export_favorites()` 方法；清理未使用的导入（`uuid`、`QGridLayout`）；移除未使用的 `mode` 参数；更新项目结构和开发规范 |
|3.18 |2026-02-18 |创建 ColorService：新建 `core/color_service.py` 模块，将 `DominantColorExtractor` 从UI层迁移到业务层；`ColorExtractInterface` 只保留UI逻辑；删除未使用的导入；提取重复代码为 `_reset_extract_button()` 方法；更新项目结构和开发规范 |
|3.17 |2026-02-18 |简化SVG智能映射逻辑：移除背景识别逻辑，透明背景总是映射到 colors[0]；所有有fill元素平等处理，按面积排序从 colors[1] 开始映射；更新第9.7.3节智能映射规则说明 |
|3.16 |2026-02-18 |优化SVG智能映射逻辑：简化透明元素处理策略（所有透明元素统一映射到 colors[0]）；修复CSS类颜色替换问题（使用内联style覆盖CSS类）；更新第9.7.3节智能映射规则说明 |
|3.15 |2026-02-18 |修复SVG智能映射逻辑：修复颜色映射顺序错误（使用元素索引而非递增索引计算配色）；修复背景检测逻辑（检测面积差异而非简单取最大）；修复语义化类型检测（区分真正语义化标识和启发式分类）；更新第9.7.3节智能映射规则说明 |
|3.14 |2026-02-18 |重构SVG配色映射逻辑：实现双模式映射策略（语义化映射模式+智能映射模式）；新增 `has_semantic_types()`、`_create_mapping_config_from_colors()`、`_apply_smart_mapping()` 方法；修改 `apply_intelligent_mapping()` 实现自动模式选择；修改 `color_preview.py` 的 `paintEvent()` 和 `get_svg_content()` 适配双模式；更新第9.7.3节映射策略规范 |
