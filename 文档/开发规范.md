...
├── core/                   # 核心功能模块目录
│   ├── __init__.py
│   ├── app_mode.py        # 应用运行模式检测模块（安装版/便携版检测、配置目录获取）
│   ├── async_loader.py    # 异步加载基类模块（BaseBatchLoader）
│   ├── color.py           # 颜色处理模块（颜色转换、明度计算、配色生成算法）
│   ├── gradient.py        # 渐变生成模块（双色渐变插值、单色明度梯度生成）
│   ├── harmony.py          # 和谐度分析模块（和谐类型判断、和谐度评分计算）
...
│   ├── tone_analysis.py    # 影调分析服务模块（ToneAnalysisService、ToneAnalysisResult，影调类型检测与明度分析）
│   └── tone_analysis_cache.py # 影调分析缓存模块（ToneAnalysisCache，影调分析结果缓存管理器，继承BaseCache）
...
│   ├── color_analysis.py   # 色彩分析界面模块（ColorAnalysisInterface）
│   ├── color_generation.py # 配色生成模块（GenerationColorInfoCard、GenerationColorPanel、ColorGenerationInterface）
│   ├── luminance_analysis.py # 明度分析界面模块（LuminanceAnalysisInterface）
│   ├── gradient_generation.py  # 渐变生成界面模块（GradientGenerationInterface、GradientCardPanel、GradientPreviewWidget）
...
│   ├── tone_analysis_dialog.py # 明度分析对话框（ToneAnalysisDialog、ChartsWidget、HistogramWidget、PieChartWidget、StatCard）
...
├── installer/             # Windows安装程序（安装版入口）
│   ├── __init__.py
│   ├── main.py            # 安装版统一入口
│   ├── wizard/            # 安装向导
│   │   ├── __init__.py
│   │   ├── install_wizard.py   # 安装向导主窗口
│   │   ├── base_page.py        # 向导页面基类
│   │   └── pages/              # 向导页面
│   │       ├── __init__.py
│   │       ├── welcome_page.py
│   │       ├── install_path_page.py
│   │       ├── progress_page.py
│   │       └── finish_page.py
│   ├── uninstaller/       # 卸载功能
│   │   ├── __init__.py
...
│       ├── install_service.py
│       ├── file_installer.py
│       ├── registry_installer.py
│       └── shortcut_installer.py
...
**路径获取函数：** 使用 `get_base_path()` 获取应用程序基础路径，支持开发环境和打包后的环境。
...
### 5.3.1 跨面板图片同步（直接信号连接，已替代 ImageMediator）
...
|  版本  |     日期     |                                    变更内容                                    |
| :--: | :--------: | :------------------------------------------------------------------------: |
| 3.59 | 2026-05-14 | 新增 Nuitka 打包兼容性要求（3.7.1节）：使用 PEP 585 内置泛型的文件必须添加 `from __future__ import annotations`，防止 Nuitka 编译后启动崩溃 |
| 3.58 | 2026-05-10 | 更新更新日志维护规范（14.2节）：新增 desc 字段说明，支持 `<br>` 分隔多行内容；更新 update_dialog.py：子标题单独一行显示，描述按 `<br>` 分割为独立行渲染 |
...
| 3.53 | 2026-04-23 | 更新项目结构（1.3节）：删除 service_factory.py，服务获取方式统一为简单工厂函数（get_xxx_service()） |
| 3.52 | 2026-04-22 | 统一面板命名规范：「色彩提取」更名为「色彩分析」，「明度提取」更名为「明度分析」，「渐变提取」更名为「渐变生成」；更新项目结构（1.3节）、多语言规范（15.2节）及相关代码示例 |
| 3.51 | 2026-04-22 | 更新资源路径规范（2.4节）：同时支持 Nuitka 和 PyInstaller 打包；更新项目结构（1.3节）：新增明度分析相关模块（tone_analysis.py、tone_analysis_cache.py、tone_analysis_dialog.py） |
| 3.50 | 2026-04-13 | 更新项目结构（1.3节）：新增 gradient.py 到 core/ 目录，包含双色渐变插值和单色明度梯度生成功能 |
| 3.49 | 2026-04-12 | 删除第11章交互设计规范（含快捷键规范）；更新后续章节编号（12-22章改为11-21章） |
| 3.47 | 2026-03-29 | 更新多语言国际化规范（16.2节）：语言包格式从 JSON 迁移到 TOML，使用 Python 3.11+ 内置 tomllib 模块 |
| 3.46 | 2026-03-27 | 更新项目结构（1.3节）：新增 app_mode.py 到 core/ 目录，支持安装版/便携版运行模式检测和配置目录区分 |
...
| 3.48 | 2026-04-04 |           新增日志系统规范（17.7节）：禁止直接使用 print 输出，避免打包后编码错误           |
