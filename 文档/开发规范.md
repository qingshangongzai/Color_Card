...
├── utils/                 # 工具函数模块目录
│   ├── __init__.py
│   ├── geometry.py        # 几何工具模块（Rect、重叠检测、覆盖判断）
│   ├── icon.py            # 图标工具模块
...
1. **元素提取**
   - 跳过特殊容器内的元素：`defs`、`clipPath`、`mask`、`filter`、`symbol`、`pattern`
   - 只提取可见的图形元素

2. **背景检测**
   - 如果最大面积元素超过画布面积的80%，识别为背景元素
   - 有背景元素时：不添加背景矩形，从 `colors[0]` 开始映射
   - 无背景元素时：添加背景矩形到 `colors[0]`，其他元素从 `colors[1]` 开始

3. **重叠检测**
   - 检测矩形元素之间的覆盖关系
   - 被完全覆盖（≥90%）的元素跳过配色分配
   - 只处理可见元素，避免颜色浪费在被遮挡的元素上

4. **透明元素处理**
   - 透明元素（无 fill 但有 stroke）：映射到 `colors[0]`

5. **有 fill 元素映射**
   - 按面积从大到小排序，面积相同时按位置排序（y坐标优先，x坐标次之）
   - 过滤掉被覆盖的元素后，从起始索引开始顺序映射
   - 相同颜色的元素共享同一个配色

6. **固定颜色处理**
   - 智能映射模式下**不处理固定颜色属性**（因为所有元素都是UNKNOWN类型）
   - 如需固定颜色，应使用语义化映射模式并添加 `data-fixed-color` 属性

7. **背景元素处理**
   - 背景元素参与配色分配，映射到 `colors[0]`
   - 背景元素不参与覆盖检测（不作为覆盖源，也不被跳过）
...
|版本 |日期 |变更内容 |
|:---:|:---:|:---:|
|3.31 |2026-02-26 |优化SVG智能映射逻辑：新增几何重叠检测功能（跳过被完全覆盖的元素）；新增特殊容器元素过滤（跳过defs/clipPath/mask等容器内的元素）；新增背景元素智能检测（面积超过画布80%识别为背景）；创建 `utils/geometry.py` 几何工具模块；更新第9.7.3节智能映射规则说明 |
