...
│   ├── generation_widgets.py  # 配色生成组件模块（GenerationColorInfoCard、GenerationColorPanel）
│   ├── palette_management_widgets.py # 配色管理组件模块（PaletteManagementColorCard、PaletteManagementCard、PaletteManagementList）
│   ├── preset_color_widgets.py # 内置色彩组件模块（PresetColorCard、PresetColorSchemeCard、PresetColorList）
│   ├── preview_widgets.py # 配色预览组件模块（DraggableColorDot、ColorDotBar、BasePreviewScene、PreviewSceneFactory、ConfigurablePreviewScene、SceneRenderer、IllustrationPreview、TypographyPreview、SVGPreviewWidget、PreviewSceneSelector、PreviewToolbar、MixedPreviewPanel）
...
配色预览功能支持通过 JSON 配置文件定义和扩展预览场景，实现场景的热插拔和动态加载。
...
|:---|:---:|:---:|:---|
|`id` |string |是 |场景唯一标识符 |
|`name` |string |是 |场景显示名称（中文） |
|`type` |string |是 |场景类型，决定使用哪个渲染类 |
|`description` |string |否 |场景描述 |
|`icon` |string |否 |图标标识 |
|`builtin` |bool |是 |是否为内置场景 |
|`config` |object |否 |场景特定配置参数 |
...
|类型 |说明 |对应类 |
|:---|:---|:---|
|`svg` |SVG自定义预览 |`SVGPreviewWidget` |
|`composite` |混合布局（2x2插画+排版） |`MixedPreviewPanel` |
|`configurable` |配置化场景（ui/web/illustration/typography/brand/poster/pattern/magazine） |`ConfigurablePreviewScene` |
|`placeholder` |占位场景（待实现） |- |
...
1. 在 `preview_scenes/scenes.json` 中添加场景定义
2. 重启应用即可生效

**方式二：新场景类型（需要代码）**


1. 创建场景类继承 `BasePreviewScene`：
...
- 用户场景保存在 `preview_scenes/user_scenes/` 目录
- 文件格式与内置场景相同
- 用户场景 ID 不能与内置场景冲突
- 应用启动时自动加载
...
|2.19 |2026-02-11 |新增SVG智能配色映射功能：创建 `core/svg_color_mapper.py` 核心模块，实现 SVG 解析、元素分类、智能配色映射；支持三种配色映射模式（顺序映射、智能映射、手动映射）；支持 CSS 类样式解析（Illustrator 导出的 SVG 常用格式）；基于元素面积的智能配色映射算法；更新 `SVGPreviewWidget` 支持新的配色映射功能；更新项目结构和开发规范 |
