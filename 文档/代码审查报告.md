...
**分层原则**：
- **界面层不直接操作数据**：所有数据处理通过业务层 Service 完成
- **业务层不依赖界面层**：core 层不导入 ui 层的任何模块
- **单向依赖**：ui → core → utils，禁止反向依赖
- **通信机制**：界面层通过调用业务层方法请求操作，业务层通过信号通知界面层更新
...
**当前问题**：业务逻辑直接写在界面类中，UI层承担了业务层的职责。
...
**建议**：抽取 Service 层，实现UI层与业务逻辑层分离

```python
# core/services/color_service.py - 业务服务
class ColorService(QObject):
    """颜色处理服务 - 纯业务逻辑，无UI依赖"""
    
    colors_extracted = Signal(list)  # 通知界面层更新
...
| **高** | 图片同步逻辑复杂 | 引入 ImageMediator 服务 | 建立业务服务，界面层只订阅展示 |
| **中** | RYB/RGB 函数重复 | 提取公共函数 | 业务层工具函数统一化 |
| **中** | 高亮逻辑重复 | 合并为通用函数 | 业务层图像处理逻辑统一 |
| **中** | 分组数据硬编码 | 移至 JSON 配置 | 数据与逻辑分离 |
| **中** | 缺少服务层 | 抽取 ColorService、PaletteService | **核心：建立业务服务层** |
...
| 方面 | 重构前 | 重构后 |
|-----|-------|-------|
| **职责** | 界面里混着业务逻辑 | 界面只负责展示，逻辑交给业务层 |
| **测试** | 必须启动UI才能测试 | 业务服务可独立单元测试 |
| **复用** | 逻辑分散无法复用 | 业务服务可被多处调用 |
| **维护** | 改一处可能影响多处 | 修改隔离，影响范围清晰 |
| **协作** | 难以分工 | 界面层和业务层可独立开发 |

### 优先处理项

建议优先处理以下两项，它们是实现分层架构的关键：

1. **`interfaces.py` 拆分**：将混合的UI和逻辑分离，UI归ui/，逻辑归core/
2. **图片同步逻辑重构**：引入 `ImageMediator` 作为业务服务，界面层只负责订阅和展示
...
├── ui/                        # 【界面层】界面组件
│   ├── main_window.py        #   主窗口
│   ├── color_extract.py      #   色彩提取界面（调用业务服务）
│   ├── luminance_extract.py  #   明度提取界面（调用业务服务）
│   └── ...                   #   其他界面
│
├── utils/                     # 【工具层】通用工具函数
│   └── ...
...
- ✅ `core/` 可以导入 `utils/`
- ❌ `core/` **禁止**导入 `ui/` 或 `dialogs/`（保证业务层独立）
- ❌ `utils/` **禁止**导入 `core/`、`ui/` 或 `dialogs/`（保证工具独立）

### 2. 图片同步重构方案（分层架构实践）

**设计目标**：将图片管理的业务逻辑从UI层剥离，形成独立的业务服务。
...
1. **单一职责**：图片加载逻辑集中在业务服务（中介者）
2. **解耦**：界面面板之间不直接通信，都通过业务服务交互
3. **可扩展**：新增界面面板只需订阅业务服务信号
4. **易测试**：业务服务可独立测试，无需启动UI
5. **状态一致**：所有界面面板看到的都是同一份业务层数据
6. **符合分层架构**：`ImageMediator` 作为业务服务，`ColorExtractInterface` 等作为界面

### 3. 业务服务延迟加载方案

**设计目标**：将非核心业务服务的初始化推迟到首次使用时，提升应用启动速度。
