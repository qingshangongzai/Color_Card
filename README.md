...
**关于本项目**：取色卡是借助 AI 编程工具（如 Trae 等）辅助开发的**非商业开源项目**。项目功能设计借鉴参考了 [Adobe Color](https://color.adobe.com/)、[色采](https://www.wxsecai.com/) 、[palettemakel](https://palettemaker.com/)等优秀的在线配色工具，旨在为中文用户提供一个**无需联网、无需注册、隐私安全、一步即达**的本地替代品。所有代码均为独立实现，与 Adobe 等公司无任何关联。
...
#### 7. SVG智能配色映射模块 (core/svg_color_mapper.py)
...
- **ElementType**：元素类型枚举
  ...
  - TEXT: 文字元素
  - STROKE: 描边元素
- **SVGElementInfo**：SVG元素信息数据类
  - 存储元素ID、标签、类名、类型、颜色、面积等信息
- **SVGElementClassifier**：SVG元素分类器
  - 基于元素标签、类名、面积自动分类元素类型
  - 支持自定义分类规则
- **SVGColorMapper**：SVG配色映射器（核心类）
  - 解析SVG文件，提取所有可着色元素
  - 支持CSS类样式解析（Illustrator导出的SVG常用格式）
  - 计算元素面积，用于智能配色映射
  - 提供三种配色映射模式：
    - **顺序映射**：按元素顺序依次应用配色
    - **智能映射**：基于元素面积和类型智能匹配配色（默认）
    - **手动映射**：支持用户手动指定元素与颜色的对应关系
  - 生成带新配色的SVG内容

#### 8. 预览场景组件模块 (ui/preview_widgets.py)
...
- **SVGPreviewWidget**：SVG自定义预览
  - 加载和显示SVG文件
  - **智能配色映射**：自动识别SVG中的配色区域（背景、主体、装饰、文字等）
