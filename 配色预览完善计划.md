...
| 场景 | 预览内容 | 优先级 |
|------|---------|--------|
| Custom | ✅ SVG自定义预览（支持导入导出） | ✅ 已实现（基础版）<br>🔄 优化中（智能配色识别） |
...
- **✅ 新增 SVG 导入功能**：支持用户导入从 Illustrator 导出的 SVG 插画文件
  ...
  - **⚠️ 待优化**：当前仅支持按顺序替换 fill/stroke 属性，需增强智能识别配色区域能力
...
4. **~~实现场景切换功能~~**（✅ 已完成）
   ...
   - ✅ 实现场景配置化系统
5. **SVG 导入功能优化**（中优先级）
   - 当前实现：按顺序替换 SVG 中的 fill/stroke 属性
   - 优化目标：
     - 智能识别 SVG 中的配色区域（如背景、主体、装饰、文字等）
     - 支持按元素类型分类应用配色（如所有背景用颜色1，所有主体用颜色2）
     - 支持用户手动选择 SVG 元素并分配颜色
     - 提供配色区域预览和编辑界面
   - 技术方案：
