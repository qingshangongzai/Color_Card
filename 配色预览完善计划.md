...
| Poster | 海报设计预览 | ✅ 已实现 | 低 |
| Pattern | 图案纹理预览 | ✅ 已实现 | 低 |
| Magazine | 杂志排版预览 | ✅ 已实现 | 低 |
...
│   ├── IllustrationPreview     # 插画预览（Mixed场景专用）
│   ├── TypographyPreview       # 排版预览（Mixed场景专用）
...
#### 1. SVG映射模式切换开关（高优先级）
- **问题描述**：代码中已实现三种配色映射模式（顺序映射、智能映射、手动映射），但UI界面缺少切换开关，用户无法选择不同的映射方式
- **当前状态**：默认使用智能映射模式，其他两种模式无法通过界面访问
- **建议方案**：
  - 在SVG预览工具栏添加映射模式选择器（下拉框或按钮组）
  - 支持实时切换映射模式并刷新预览
  - 记忆用户选择的映射模式偏好
- **相关代码位置**：
  ```python
  # ui/preview_widgets.py
  - SVGPreviewWidget.MODE_SEQUENTIAL / MODE_INTELLIGENT / MODE_MANUAL
  - SVGPreviewWidget.set_mapping_mode() / get_mapping_mode()
  ```

#### 2. SVG元素替换问题（高优先级）
- **问题描述**：部分SVG图片的元素替换存在问题，某些元素的颜色无法正确应用新配色
- **可能原因**：
  - CSS样式类解析不完整（如多个class组合、内联样式优先级问题）
  - 元素选择器匹配失败（某些元素无法被正确识别和定位）
  - 颜色属性继承问题（子元素继承父元素颜色，但替换时未考虑）
  - 渐变、图案填充等特殊填充类型的处理不完善
- **需要进一步测试的SVG类型**：
  - 包含复杂CSS样式的SVG（如多个class组合、伪类选择器）
  - 使用渐变填充（linearGradient、radialGradient）的SVG
  - 使用图案填充（pattern）的SVG
  - 包含嵌套变换和群组（g元素）的复杂SVG
- **建议方案**：
  - 增强CSS样式解析器，支持更复杂的CSS选择器
  - 完善元素匹配逻辑，确保所有可着色元素都能被正确识别
  - 添加对渐变、图案等特殊填充类型的支持
  - 提供调试模式，显示哪些元素被成功替换，哪些失败
- **相关代码位置**：
  ```python
  # core/svg_color_mapper.py
  - SVGColorMapper._parse_css(): CSS样式解析
  - SVGColorMapper._extract_elements(): 元素提取
  - SVGColorMapper.apply_intelligent_mapping(): 颜色应用
  ```
...
- 保持代码风格与现有代码一致
- 添加适当的文档字符串和注释
- **代码清理已完成**：删除了废弃的 `MobileUIPreview` 和 `WebPreview` 类，减少了约 400 行冗余代码
